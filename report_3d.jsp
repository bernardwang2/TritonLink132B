<html>
<HEAD><TITLE>CSE132B Webapp: Report 3d</TITLE></HEAD>
<BODY>  <FONT SIZE="5">Report 3d</FONT> <P />
...
<table>
    ...
    <tr>
        <td>
            <table>
                <form action="report_3d.jsp" method="POST">
                <input type="hidden" name="action" value="submit"/>

                <td>
                    <p>Display the grade point average that professor: </p>
                </td>
                ...
                <td>
                    <p> gave to the students taking course: </p>
                </td>
...
<%
    ...
    if (action != null && action.equals("submit")) {
        ...
        pstmt = conn.prepareStatement("SELECT CAST(AVG(g.number_grade) AS DECIMAL(10, 3)) AS gpa " +
                                      "FROM academic_history_new r, grade_conversion g " +
                                      "WHERE r.grade = g.letter_grade AND r.class_id IN " +
                                      "  (SELECT c.class_id " +
                                      "   FROM classes c " +
                                      "   WHERE c.instructor = ? AND c.class_name = ?) " +
                                      "GROUP BY r.grade_opt"
                                      );
...
%>
    ...
    <tr>
        <td>
            Professor <%= selected_professor %> has given the following GPA to the students taking course <%= selected_course %> over the years
        </td>
    </tr>
